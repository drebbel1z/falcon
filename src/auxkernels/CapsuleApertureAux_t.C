#include "CapsuleApertureAux_t.h"

registerMooseObject("FalconApp", CapsuleApertureAux_t);

InputParameters
CapsuleApertureAux_t::validParams()
{
...
CapsuleApertureAux_t::CapsuleApertureAux_t(const InputParameters &parameters)
...
Real CapsuleApertureAux_t::computeValue()
{
    ...
    */
    ...
    else if (_t >= 114501 && _t < 143000)
    {
        some_scale = 0.75;
    }
    else if (_t >= 143000 && _t < 186001)
    {
        some_scale = 0.5;
    }
    else if (_t >= 186001 && _t < 247001)
    {
        some_scale = 0.35;
    }
    else if (_t >= 247001)
    ...
    Real sigmoid_scale = 1.0 / (1.0 + std::exp(_slope_at_midpoint * (distance_from_center_line - some_scale * _midpoint_of_sigmoid)));
